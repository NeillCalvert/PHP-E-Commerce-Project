<!--Users who have not logged in and have a session variable may not enter this page-->
<?php
session_start();
include('conn.php');
...
$query = "SELECT marketplaceid,imagepath, name, price, status, firstname, consolename, postdate FROM marketplace INNER JOIN users ON marketplace.sellerid = users.userid INNER JOIN consoles ON marketplace.consoleid = consoles.consoleid INNER JOIN itemstatus ON marketplace.statusid = itemstatus.itemstatusid  WHERE consoles.consoleid = '$categoryid'ORDER BY postdate DESC";
