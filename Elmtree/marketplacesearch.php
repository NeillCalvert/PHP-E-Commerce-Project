...
$query = "SELECT marketplaceid,imagepath, name, price, status, firstname, consolename, postdate FROM marketplace INNER JOIN users ON marketplace.sellerid = users.userid INNER JOIN consoles ON marketplace.consoleid = consoles.consoleid INNER JOIN itemstatus ON marketplace.statusid = itemstatus.itemstatusid  WHERE name LIKE  '%$searchinput%' OR consolename LIKE '%$searchinput%' ORDER BY postdate DESC";
...
<div class="container">
  ...
  if($result->num_rows > 0){
     ...
     echo "<h1 class='text-danger'>Oh dear looks like nothing matches that search please try again later or search for something else</h1>";
   }
       ?>
  <!--Start of footer-->
